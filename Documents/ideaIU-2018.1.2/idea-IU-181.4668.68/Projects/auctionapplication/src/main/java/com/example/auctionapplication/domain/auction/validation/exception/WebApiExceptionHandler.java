package com.example.auctionapplication.domain.auction.validation.exception;


import com.example.auctionapplication.domain.auction.validation.rule.RuleFailedException;
...
public class WebApiExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    public ResponseEntity catchAll(Exception ex, WebRequest request){
    ...
    public ResponseEntity<ApiError> ruleFailedException(RuleFailedException ex, WebRequest request) {
        return wrap(request, new ResponseEntity("Rule Fail Error.", new HttpHeaders(), HttpStatus.BAD_REQUEST), ex, ex.getBindingResult());//e.getBindingResult().toString());
    }
...
//Make this extend the ResponseEntityExceptionHandler
//Have a common ApiMessage for all these types

//NOTE TO SELF 'NEED CUSTOM RuleFailedException that throws an Error Message which is indicative of the error. Right now, I don't think I'm doing anything with the binding result'.
//Think that's what Steven wants ApiError to do?
